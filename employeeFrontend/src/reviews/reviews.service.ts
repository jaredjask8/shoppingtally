...
export class ReviewsService {
  ...
  sendReview(review:Review):Observable<Review[]>{
    ...
    return this.http.post<Review[]>("https://shoppingtally.click/api/v1/reviews", review, {headers});
  }

  getReviews():Observable<Review[]>{
    const headers = new HttpHeaders().set('Authorization', 'Bearer ' + this.userService.getEnvironment().token).set('Access-Control-Allow-Origin', '*');
    return this.http.post<Review[]>("https://shoppingtally.click/api/v1/reviews/all", null, {headers});
  }
}
