...
export class RegisterService {
  ...
  getUser(token:string):Observable<JwtUserResponse>{
    return this.http.post<JwtUserResponse>("https://192.168.0.209:8080/api/v1/auth/user", {token:token});
  }

  authLogin(email:string, password:string):Observable<TokenResponse>{
    return this.http.post<TokenResponse>("https://192.168.0.209:8080/api/v1/auth/authenticate", {email:email,password:password})
  }


  getUsers():Observable<JwtUserResponse>{
    return this.http.get<JwtUserResponse>("192.168.0.209:8080/api/user/all");
  }

  registerUser(user: User){
    return this.http.post("https://192.168.0.209:8080/api/v1/auth/register", user);
  }
